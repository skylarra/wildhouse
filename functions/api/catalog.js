...
import { squareConfig, squareFetch, squareToCatalog, json } from "./_square.js";

export async function onRequestGet({ env }) {
  const cfg = squareConfig(env);
  if (!cfg.configured) {
    return json({ error: "Square not configured", fallback: true }, 501);
  }
  ...
  try {
    ...
    return json(squareToCatalog(objects, counts));
  } catch (err) {
