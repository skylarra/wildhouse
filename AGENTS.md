...
- **Collections vs categories:** Customer-facing collections are driven by the Square Catalog custom attribute named `Collection`. Square Categories are product types (Shop filters / collection type chips). Admin (`/admin/collections`) controls **visibility, featured, order, description, images** (Cloudflare KV `COLLECTIONS_CONFIG`). Membership is never edited in admin.
...
- Admin: `/admin/collections` (KV saves) + `/admin/products` (read-only Square Featured / Collection inspector). Set `ADMIN_PASSWORD` and bind KV `COLLECTIONS_CONFIG`.
- Homepage merchandising: Featured Collections (visible + featured + products); Featured Products (Square Featured attribute). `getBestSellingProducts()` is reserved for real sales data and returns empty pre-launch.
